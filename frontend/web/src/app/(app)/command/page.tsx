      ...
      <div>
        ...
        <p className="mt-0.5 text-xs text-fgMuted">
          ...
          {t.linkedRef ? (
            <>
              {' '}
              · <span className="font-mono">{t.linkedRef}</span>
            </>
          ) : null}
        </p>
      </div>
...
export default async function CommandPage() {
  ...
  if (res.kind === 'denied') {
    return (
      <div className="py-16 text-center">
        <h1 className="text-lg font-semibold text-fg">Command Center</h1>
        <p className="mt-2 text-sm text-fgMuted">
          This executive view is available to leadership only.
        </p>
      </div>
    );
  }
  if (res.kind !== 'ok') {
    return (
      <div className="py-16 text-center text-sm text-fgMuted">
        Could not load the command overview. Please retry.
      </div>
    );
  }
  const o = res.data;
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-xl font-bold text-fg">Command Center</h1>
        <p className="text-sm text-fgMuted">
          What needs your attention · next {o.horizonDays} days
        </p>
      </header>
      ...
      <section>
        ...
        <Card>
          {o.needsAttention.length === 0 ? (
            <p className="py-4 text-center text-sm text-fgSubtle">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
              ...
              o.upcomingDeadlines.grants.map((g) => (
                ...
                >
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-fgMuted">
                      {g.funder} · {fmtMoney(g.amountMinor, g.currency)} · due{' '}
                      {fmtDate(g.deadlineAt)}
                    </p>
                  </div>
        ...
        <section>
          ...
          <Card>
            {pipeline.length === 0 ? (
              <p className="py-4 text-center text-sm text-fgSubtle">No grants yet.</p>
            ) : (
              pipeline.map(([status, n]) => (
                ...
                >
                  <span className="text-sm capitalize text-fgMuted">{status}</span>
                  <span className="text-sm font-semibold tabular-nums text-fg">{n}</span>
                </div>
              ))
            )}
          </Card>
      ...
      <section>
        ...
        <Card>
          {o.myOpenTasks.length === 0 ? (
            <p className="py-4 text-center text-sm text-fgSubtle">No tasks assigned to you.</p>
          ) : (
