...
export const chooseRole = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) =>
    z.object({ role: z.enum(["rider", "driver", "advertiser"]) }).parse(d),
  )
  .handler(async ({ data, context }) => {
    const { admin } = await import("./uza.server");
    const db = admin();
    ...
    if (data.role === "driver") {
      ...
      if (driver) {
        await db
          .from("savings_rules")
          .upsert({ driver_id: driver.id }, { onConflict: "driver_id" });
      }
    }
    return { ok: true };
  });
...
export const getDriverDocs = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: driver } = await db
      .from("drivers")
      .select(
        "id_photo_url, licence_photo_url, insurance_photo_url, vehicle_photo_url, vest_photo_url, photo_url, insurance_expiry",
      )
