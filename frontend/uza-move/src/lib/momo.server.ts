...
export async function settleMomoCollection(
  ...
  if (!claimed) {
    const { data: existing } = await db
      ...
      .maybeSingle();
    return existing
      ? { ok: true as const, already: true }
      : { ok: false as const, error: "unknown_reference" };
  }
  const momo = claimed;

  if (outcome !== "successful") {
    if (momo.trip_id)
      await db.from("trips").update({ pay_status: "failed" }).eq("id", momo.trip_id);
  ...
  if (!momo.trip_id) {
    if (momo.direction === "collection" && momo.initiated_by) {
      await post(db, momo.initiated_by, "topup", Number(momo.amount), {
        delta: Number(momo.amount),
        ref: `momo:${externalRef}`,
        note: "Kongeramo amafaranga (MoMo)",
      });
    }
  ...
  await db
    .from("trip_events")
    .insert({ trip_id: trip.id, event_type: "payment_verified", meta: { externalRef } });
  ...
  const ledger = await applyTripLedger(
    ...
    {
      id: trip.id,
      final_fare: Number(trip.final_fare ?? trip.quoted_fare),
      commission_amount: Number(trip.commission_amount),
      pay_method: "momo",
    },
    driver,
  );
...
export async function settleMomoDisbursement(
  externalRef: string,
  outcome: "successful" | "failed",
  payload: Record<string, unknown>,
) {
  const db = admin();
  const { data: momo } = await db
    .from("momo_transactions")
    .select("*")
    .eq("external_ref", externalRef)
    .maybeSingle();
  ...
  if (outcome === "failed" && momo.initiated_by) {
    ...
    await post(db, momo.initiated_by, "topup", Number(momo.amount), {
      delta: Number(momo.amount),
      ref: `momo:${externalRef}`,
      note: "Gukura amafaranga byanze — asubijwe",
    });
  }
