...
| 9 | Add `GET` list/work-queue endpoints (scoped + masked) — `GET /quotations?owner=…`, `/orders?…`, and `GET /projects/:ref`. See `docs/contract-requests/2026-07-29-web-list-endpoints.md`. | frontend web slice, 2026-07-29 | next web sprint | The dashboard currently keeps a per-user worklist cookie of refs because only GET-by-ref exists; a real "everything I own" queue needs these. |
| 10 | Web: expand beyond the vertical slice — remaining role dashboards (Cecilia/François/Adeline/Finance/agent/customer/Imari), payment capture UI, quotation-revise UI, and the offline inspection/receiving flow. | web slice handoff, 2026-07-29 | future web sprints | The slice proves the stack; `docs/handoff/web-slice.md` lists the suggested build order. |
