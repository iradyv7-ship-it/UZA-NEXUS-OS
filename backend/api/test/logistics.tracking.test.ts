...
describe('CF-023 — delay fans out to five distinct parties', () => {
  it('notifies client, agent, owner, front office and partner, and publishes shipment.delayed', async () => {
    const s = await bookShipment();
    // Booking already sent the container-confirmed notice on this subject; only the rows the
    // delay adds are under test here.
    const alreadySent = await prisma.notification.findMany({ where: { subjectRef: s.ref } });
    ...
    const sentBefore = new Set(alreadySent.map((n) => n.id));
    const notes = (await prisma.notification.findMany({ where: { subjectRef: s.ref } })).filter(
      (n) => !sentBefore.has(n.id),
    );
