 ...
 *
 * WHY IT RUNS HERE (backend/api), not in backend/worker: fan-out invokes NestJS-managed feature
 * services (OrderService, InvoiceService, …) whose constructor injection depends on
 * `emitDecoratorMetadata`. backend/api runs under SWC, which emits that metadata; backend/worker
 * runs under tsx (esbuild), which does NOT — under tsx the DI container yields `undefined`
 * constructor args. So the consumer lives where
 * the container and the services live. backend/worker keeps the PUBLISHER role only (drain the
