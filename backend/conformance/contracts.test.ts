 ...
 *
 *   node --experimental-strip-types backend/conformance/contracts.test.ts
 *
 * Assertions that need a database live in backend/api/test.
 */
...
test('CF-002/003: quoted margin holds at sell incoterm, DAP margin is lower and visible', () => {
  const l = emptyLadder();
  l.exw.estMinor = minor(41.0);
  l.inlandCn.estMinor = minor(1.2);
  l.exportDocs.estMinor = minor(0.35);
  l.originThc.estMinor = minor(0.6);
  l.ocean.estMinor = minor(7.09); // 6.50 + 9% contingency
  l.insurance.estMinor = minor(0.3);
  l.destCharges.estMinor = minor(1.8);
  l.dutyVat.estMinor = minor(4.9);
  ...
  assert.ok(
    dapMargin(l, sell) < marginAt(l, sell, 'CIF') - 0.1,
...
test('CF-027: an agent reaches only the customers in their scope', () => {
  // `customer` is not a login role (a customer never authenticates into Nexus), so object
  // scope is exercised on the narrowest internal role: an agent scoped to named customers.
  assert.equal(inScope(agent, { customerId: 'CUS-CD-000001' }), true);
  assert.equal(inScope(agent, { customerId: 'CUS-RW-000999' }), false);
  assert.equal(inScope(agent, { agentId: agent.userId }), true, 'own records are always in scope');
});
...
test('a partner reaches a shipment by ref, and a delivery by its shipmentRef', () => {
  ...
  assert.equal(
    inScope(partner, { shipmentRef: 'SHP-2026-0001' }),
    true,
    'delivery hangs off a shipment',
  );
