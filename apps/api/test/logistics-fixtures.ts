...
export const inspectionRecorded = (
  opts: {
    poRef?: string;
...
export const qualityFailed = (
  opts: { poRef?: string; eventId?: string } = {},
): EventEnvelope<'quality.failed'> => ({
  eventId: opts.eventId ?? randomUUID(),
  name: 'quality.failed',
  actorId: 'FRANCOIS',
  occurredAt: new Date().toISOString(),
 ...
 */
export async function loadableLot(
  opts: {
    orderRef?: string;
    poRef?: string;
  ...
  if (opts.payPreLoading !== false) {
    await orderPayments.handlePaymentVerified(
      paymentVerified({ orderRef, trigger: 'confirmation', paidFraction: 0.5 }),
    );
    await orderPayments.handlePaymentVerified(
      paymentVerified({ orderRef, trigger: 'pre_loading', paidFraction: 1.0 }),
    );
  }
