...
describe('partner shipments list — scope in, cost masked', () => {
  it('lists ONLY the caller’s in-scope shipments, never one outside scope', async () => {
    const a = await bookShipment({ orderRef: 'ORD-BULK-2026-0001', poRef: 'PO-CN-2026-0001', container: 'MSKU-A' });
    const b = await bookShipment({ orderRef: 'ORD-BULK-2026-0002', poRef: 'PO-CN-2026-0002', container: 'MSKU-B' });

    // Imari is assigned only to shipment A.
    const imari = partner([a.shipment.ref]);
    const rows = (await partnerPortal.listShipments(imari, { limit: 20, offset: 0 })) as Record<string, unknown>[];
  ...
  it('an internal role listing the same shipment sees the freight cost (not masked)', async () => {
    const a = await bookShipment({ orderRef: 'ORD-BULK-2026-0001', poRef: 'PO-CN-2026-0001', container: 'MSKU-A' });
    const rows = (await partnerPortal.listShipments(vm, { limit: 20, offset: 0 })) as Record<string, unknown>[];
    const row = rows.find((r) => r.ref === a.shipment.ref)!;
  ...
  it('a partner with no assigned shipments lists nothing (empty scope ⇒ empty page)', async () => {
    await bookShipment({ orderRef: 'ORD-BULK-2026-0001', poRef: 'PO-CN-2026-0001', container: 'MSKU-A' });
  ...
  it('honours limit/offset with a stable updatedAt-desc sort', async () => {
    ...
    const page1 = (await partnerPortal.listShipments(imari, { limit: 1, offset: 0 })) as Record<string, unknown>[];
    const page2 = (await partnerPortal.listShipments(imari, { limit: 1, offset: 1 })) as Record<string, unknown>[];
...
describe('partner shipments list agrees with inScope (the predicate is a mirror, not a re-implementation)', () => {
  it('every listed shipment passes inScope for the caller, and an out-of-scope ref is absent', async () => {
    ...
    const imari = partner([a.shipment.ref]);
    const rows = (await partnerPortal.listShipments(imari, { limit: 100, offset: 0 })) as Record<string, unknown>[];
  ...
  it('a role with no shipment:read grant is denied 403 before the predicate runs', async () => {
    ...
    const cust: Actor = { userId: 'CUS-1', role: 'customer', office: 'GOM', scope: { customerId: 'CUS-CD-000001' } };
