...
import { suppliers, offers, cecilia, agent, francois, UNIT_COST, acceptedOffer } from './sourcing-quality-fixtures';
...
describe('sourcing — supplier self-service channel (staff-relayed)', () => {
  ...
  it('masks supplier cost from a role without the supplierQuote grant', async () => {
    const { offer } = await acceptedOffer();
    const asAgent = await offers.read(agent, offer.ref);
    expect(asAgent.supplierUnitCost).toBe('***');
  });
