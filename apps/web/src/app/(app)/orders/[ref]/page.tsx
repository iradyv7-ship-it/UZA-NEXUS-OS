  ...
  return (
    <DetailShell>
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-fg">{t('ord.title')}</h1>
          <p className="font-mono text-xs text-fgSubtle">{o.ref}</p>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2 lg:items-start">
        <Card>
          <dl className="divide-y divide-border">
            ...
            <Field label={t('ord.tier')}>
              {o.tier === 'established' ? t('ord.tier.established') : t('ord.tier.new')}
            </Field>
        ...
        <div>
          <h2 className="mb-2 text-sm font-semibold text-fgMuted">{t('ord.schedule')}</h2>
          ...
          {installments ? (
            ...
            <ul className="space-y-2">
              {o.installments.map((inst) => (
                <li key={inst.ref} className="rounded-xl border border-border bg-surface p-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-fg">
                        {t(`trigger.${inst.trigger}`)}
                      </p>
                      <p className="text-xs text-fgMuted">{percent(inst.pct, locale)}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-semibold tabular-nums text-fg">
                        {money(inst.amountMinor, locale)}
                      </p>
      ...
      <Link
        ...
        className="block rounded-lg border border-border px-4 py-3 text-center text-sm font-medium text-fgMuted sm:inline-block sm:w-auto sm:px-8"
      >
        {t('ord.viewQuotation')}
      </Link>
    </DetailShell>
...
function InstallmentRow({
  ...
  return (
    <li className="rounded-xl border border-border bg-surface p-3">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-medium text-fg">{t(`trigger.${inst.trigger}`)}</p>
          <p className="text-xs text-fgMuted">{percent(inst.pct, locale)}</p>
        </div>
        <div className="text-right">
          <p className="text-sm font-semibold tabular-nums text-fg">
      ...
      {!settled && !pending && canPay && (
        ...
        >
          ...
          <label htmlFor={`proof-${inst.ref}`} className="block text-xs font-medium text-fgMuted">
            {t('pay.proofLabel')}
          </label>
          <input
            id={`proof-${inst.ref}`}
            ...
            className="w-full rounded-lg border border-border px-3 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/30"
          />
          ...
          <p className="text-[11px] text-fgSubtle">{t('pay.proofHint')}</p>
        </form>
      )}
    </li>
  );
