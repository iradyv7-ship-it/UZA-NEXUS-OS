...
const point = z.object({ lat: z.number(), lng: z.number(), address: z.string().max(200).optional() });
...
export const quoteTrip = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) => z.object({ pickup: point, dropoff: point, vehicle_type: vehicle }).parse(d))
  .handler(async ({ data }) => {
    ...
    const { data: tariff } = await db
      .from("fare_tariffs").select("*").eq("vehicle_type", data.vehicle_type).eq("active", true).limit(1).maybeSingle();
...
export const requestTrip = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: trip, error } = await db.from("trips").insert({
      ...
      pickup_lat: data.pickup.lat, pickup_lng: data.pickup.lng, pickup_address: data.pickup.address ?? null,
      dropoff_lat: data.dropoff.lat, dropoff_lng: data.dropoff.lng, dropoff_address: data.dropoff.address ?? null,
      distance_km: q.distanceKm, duration_min: q.durationMin,
      quoted_fare: q.fare, commission_amount: q.commission, driver_earnings: q.driverEarnings,
      start_pin: makePin(),
    ...
    await db.from("trip_events").insert({
      trip_id: trip.id, event_type: "requested", lat: data.pickup.lat, lng: data.pickup.lng, actor_id: context.userId,
    });
...
export const getTrip = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: driverProfile } = driver
      ? await db.from("profiles").select("full_name, avatar_url, phone").eq("id", driver.user_id).maybeSingle()
      : { data: null };

    const isRider = trip.rider_id === context.userId;
    const isDriver = driver?.user_id === context.userId;
    if (!isRider && !isDriver) {
      const { data: ops } = await db.from("user_roles").select("role").eq("user_id", context.userId);
...
export const listOpenTrips = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { offerOpenTo, offerToNearestDriver, expireStaleTrips, isInsured } = await import("./dispatch.server");
    const db = admin();
    await expireStaleTrips(db);
    const { data: driver } = await db.from("drivers").select("*").eq("user_id", context.userId).maybeSingle();
    if (!driver || !driver.is_online || driver.status !== "approved" || !isInsured(driver)) return [];
    ...
    const { data: trips } = await db
      .from("trips").select("*").eq("status", "requested").eq("vehicle_type", driver.vehicle_type)
      .order("requested_at", { ascending: true }).limit(25);
    ...
    return rows
      .map((t) => ({
        ...t,
        is_offered_to_me: t.offered_driver_id === driver.id && t.offered_until != null && new Date(t.offered_until).getTime() > now,
        pickup_distance_km:
          driver.current_lat != null && driver.current_lng != null
            ? Math.round(haversineKm({ lat: driver.current_lat, lng: driver.current_lng }, { lat: t.pickup_lat, lng: t.pickup_lng }) * 10) / 10
            : null,
      }))
      .sort((a, b) => {
...
export const acceptTrip = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!isInsured(driver)) throw new Error("Your insurance has expired. Upload a valid certificate to keep driving.");
    const { assertCommissionClear } = await import("./dispatch.server");
    await assertCommissionClear(db, driver.user_id);

    const { data: pending } = await db.from("trips").select("*").eq("id", data.trip_id).maybeSingle();
    if (!pending) throw new Error("Urwo rugendo ntirubonetse.");
    if (!offerOpenTo(pending, driver.id)) throw new Error("This trip was offered to a closer driver first.");

    const { data: trip, error } = await db
      .from("trips").update({ driver_id: driver.id, status: "accepted", accepted_at: new Date().toISOString(), offered_driver_id: null, offered_until: null })
      .eq("id", data.trip_id).eq("status", "requested").select("*").maybeSingle();
    if (error) throw error;
    if (!trip) throw new Error("Urwo rugendo rwafashwe n'undi mushoferi.");

    await db.from("trip_events").insert({
      trip_id: trip.id, event_type: "accepted", lat: driver.current_lat, lng: driver.current_lng, actor_id: context.userId,
    });
    return trip;
  });
...
export const setTripStatus = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.status === "arriving" && !isDriver) throw new Error("Only the driver can mark arrival.");
    if (data.status === "cancelled_by_rider" && !isRider) throw new Error("Only the rider can cancel as rider.");
    ...
    await db
      ...
      .update({
        ...
        ...(isCancel
          ? { final_fare: 0, driver_earnings: 0, commission_amount: 0, cancellation_fee: fee, cancelled_by: context.userId }
          : {}),
      })
      .eq("id", trip.id);
    await db.from("trip_events").insert({
      trip_id: trip.id, event_type: data.status, lat: data.lat ?? null, lng: data.lng ?? null, actor_id: context.userId,
...
export const startTrip = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) =>
    z.object({ trip_id: z.string().uuid(), pin: z.string().length(4), lat: z.number().optional(), lng: z.number().optional() }).parse(d),
  )
  .handler(async ({ data, context }) => {
    const { admin } = await import("./uza.server");
    const db = admin();
    ...
    await db.from("trips").update({ status: "started", started_at: new Date().toISOString() }).eq("id", trip.id);
    await db.from("trip_events").insert({
      trip_id: trip.id, event_type: "started", lat: data.lat ?? null, lng: data.lng ?? null, actor_id: context.userId,
    });
...
export const completeTrip = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await db.from("trip_events").insert({
      trip_id: trip.id, event_type: "completed", lat: data.lat ?? null, lng: data.lng ?? null, actor_id: context.userId,
    });
...
export const requestMomoPayment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { externalRef, simulated } = await requestToPay({
      provider: data.provider, phone: data.phone, amount, reference: trip.id.slice(0, 8),
    });

    const { data: momo, error } = await db.from("momo_transactions").insert({
      provider: data.provider, direction: "collection", phone: data.phone, amount,
      external_ref: externalRef, trip_id: trip.id, initiated_by: context.userId,
    }).select("*").single();
    if (error) throw error;
...
export const simulateMomoCallback = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) =>
    z.object({ external_ref: z.string().min(4), outcome: z.enum(["successful", "failed"]) }).parse(d),
  )
  .handler(async ({ data }) => {
...
export const rateTrip = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) =>
    z.object({ trip_id: z.string().uuid(), stars: z.number().int().min(1).max(5), comment: z.string().max(300).optional() }).parse(d),
  )
  .handler(async ({ data, context }) => {
    const { admin } = await import("./uza.server");
    const db = admin();
    ...
    await db.from("ratings").upsert(
      { trip_id: trip.id, rater_id: context.userId, ratee_id: rateeId, stars: data.stars, comment: data.comment ?? null },
    ...
    if (row) {
      ...
      await db.from(table).update({ rating_count: count, rating_avg: Math.round(avg * 100) / 100 }).eq("id", row.id);
    }
    return { ok: true };
  });
...
export const raiseIncident = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { admin } = await import("./uza.server");
    await admin().from("incidents").insert({ ...data, reporter_id: context.userId });
    return { ok: true };
  });
...
export const myTrips = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const query = db.from("trips").select("*").order("requested_at", { ascending: false }).limit(40);
    const { data } = driver
